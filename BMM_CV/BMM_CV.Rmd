...
```{r}
pubs <- RefManageR::ReadBib(here::here("BMM_CV/refs_articles.bib"), check = FALSE) |> 
  as_tibble() |> 
...
```{r}
bibliography_entries(here::here("BMM_CV/refs_theses.bib")) |> 
  arrange(issued)
```
...
```{r}
bibliography_entries(here::here("BMM_CV/refs_articles.bib")) |> 
...
```{r}
bibliography_entries(here::here("BMM_CV/refs_reports.bib")) %>%
  arrange(issued, desc(author$family))
```
...
```{r}
bibliography_entries(here::here("BMM_CV/Rpackages.bib"))
```
